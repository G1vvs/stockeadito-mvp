...
def register_sale(sale_data: SaleRequest, user = Depends(get_current_user)):
    ...
    try:
        ...
        for item in sale_data.items:
            ...
            .select("stock_actual, precio_venta, catalogo_universal(nombre)")\
            .eq("user_id", user.id)\
            ...
            if current_stock < item.quantity:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                                    detail=f"stock insuficiente para {product_name}. Tines {current_stock}, intentas vender {item.quantity}")
        ...
        sale_response = supabase.table("venta").insert(new_sale).execute()
